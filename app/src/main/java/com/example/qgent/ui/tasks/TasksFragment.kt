...
class TasksFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        taskListViewModel.uiState.observe(viewLifecycleOwner) { state ->
            ...
            setSkeletonLoading(binding.viewSkeletonTaskList.root, !tasksLoaded && state.myTasks.isEmpty())
            // 空状态：列表为空时展示提示（首次加载完成后再显示，避免加载中与骨架并存）
            binding.tvRecentTaskEmpty.isVisible = tasksLoaded && recent.isEmpty()
            binding.tvTaskEmpty.isVisible = tasksLoaded && state.myTasks.isEmpty()
    ...
    private fun loadMyTasks() {
        ...
        taskListViewModel.loadMyTasks(projectId, com.example.qgent.data.SessionStore.user()?.id, onDone = ::markTasksLoaded)
    }
